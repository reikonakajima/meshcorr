//	$Id: Random.h,v 1.4 2003-06-10 20:52:41 garyb Exp $
...
class GaussianDeviate {
 public:
  GaussianDeviate(): u(), iset(0) {}; //seed with time
  GaussianDeviate(const long lseed): u(lseed), 
    iset(0) {} //seed with specific number
  float operator() () {return newvalue();}
  operator float() {return newvalue();}
...
class ExponentialDeviate {
 public:
  ExponentialDeviate(): u() {} //seed with time
  //seed with specific number:
  ExponentialDeviate(const long lseed): u(lseed) {} 
  float operator() () {return newvalue();}
  operator float() {return newvalue();}
  void Seed() {u.Seed();}
...
class BinomialDeviate {
 public:
  //seed with time:
  BinomialDeviate(const int _N, const double _p): u(), N(-1) {init(_N,_p);}
  //seed with specific number
  BinomialDeviate(const int _N, const double _p,
		  const long lseed): u(lseed), N(-1) {init(_N,_p);} 
...
class PoissonDeviate {
 public:
  //seed with time:
  PoissonDeviate(const double _mean): u() {init(_mean);}
  //seed with specific number
  PoissonDeviate(const double _mean, const long lseed): u(lseed) {init(_mean);} 
  //Reset the parameters
...
using ran::UniformDeviate;
using ran::BinomialDeviate;
using ran::PoissonDeviate;
using ran::ExponentialDeviate;
using ran::GaussianDeviate;
using ran::CGaussianDeviate;
